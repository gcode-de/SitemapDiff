...
const Header: React.FC<HeaderProps> = ({login, logout, user}:HeaderProps) => {
    return (
        <AppBar position="static">
            <Toolbar sx={{ display: 'flex', justifyContent: 'space-between', minHeight: 42 }}>
                ...
                {user ? <Button color="inherit" onClick={logout}>Logout</Button>  :
                    <Button color="inherit" onClick={login}>Login with <img src={google} alt="Google logo" width={16} style={{ marginLeft: 8 }}/></Button>}
